...
void generate_synthetic_data(DType dist, int num_keys, std::vector<KT>& keys, 
                            std::string path = "") {
...
template<typename KT, typename VT>
void load_workloads(std::string path, std::vector<std::pair<KT, VT>>& load_data, 
  ...
  load_data.reserve(num_reqs);
  run_reqs.reserve(num_reqs);
  for (int i = 0; i < num_reqs; ++ i) {
    Request<KT, VT> req;
    in.read((char*)&req, sizeof(Request<KT, VT>));
    if (req.op == kBulkLoad) {
      load_data.push_back(req.kv);
    } else {
      run_reqs.push_back(req);
    }
  }
  in.close();
}
...
void assess_data(const std::pair<KT, VT>* kvs, uint32_t size, bool pretty=false) {
  ...
  if (num_unordered == 0) {
    ...
    uint32_t max_conflicts = ci->conflicts[ci->num_conflicts - 1];
    uint32_t tail_conflicts = ci->conflicts[std::max(0, int(ci->num_conflicts * 0.99) - 1)];
    double avg_conflicts = 0;
    for (uint32_t i = 0; i < ci->num_conflicts; ++ i) {
      avg_conflicts += ci->conflicts[i];
    }
    avg_conflicts /= ci->num_conflicts;
